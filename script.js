                ...
                try {
                    ...
                    if (projectData.rows && Array.isArray(projectData.rows)) {
                        projectData.rows.forEach(data => {
                            ...
                            if (theoreticalQuantityInput && data.quantiteTheorique) {
                                theoreticalQuantityInput.value = data.quantiteTheorique;
                            }

                            // Set Produced Quantity
                            const producedQuantityInput = row.querySelector('.quantite-produite');
                            if (producedQuantityInput && data.quantiteProduite) {
                                producedQuantityInput.value = data.quantiteProduite;
                            }
                    ...
                    console.error('Error parsing BGT file:', error);
                    alert('Erreur lors de la lecture du fichier BGT. Assurez-vous que le fichier est valide.');
                }
            };
            reader.readAsText(file);
        } catch (error) {
            console.error('Error importing BGT file:', error);
            alert('Erreur lors de l\'importation du fichier BGT.');
        }
    });
}
...
'Zodid 50'
    ...
    function showAllSuggestions(input, suggestions, isFormeInput = false) {
        ...
        sortedSuggestions.slice(0, 5).forEach(suggestion => {
...
function exportTableToExcel() {
    ...
    link.download = `Inventaire_${today.getFullYear()}-${today.getMonth() + 1}-${today.getDate()}.bgt`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const produitHeader = document.getElementById('produit-header');
    const sortIndicator = document.getElementById('sort-indicator');

    produitHeader.addEventListener('click', () => {
        const tableBody = document.getElementById('product-table-body');
        const rows = Array.from(tableBody.querySelectorAll('tr:not(.total-row)'));

        rows.sort((a, b) => {
            const productA = (a.querySelector('.produit-input')?.value || '').trim().toLowerCase();
            const productB = (b.querySelector('.produit-input')?.value || '').trim().toLowerCase();
